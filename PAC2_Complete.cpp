            ...
            for (int x = 0; x < tilemap.tileCountX; x++)
            {
                ...
                if ((tilemap.tiles[y * tilemap.tileCountX + x].collider == 0) &&
                    CheckCollisionRecs(Rectangle { player.x + PLAYER_COLLISION_PADDING, player.y + PLAYER_COLLISION_PADDING, player.width, player.height },
                        Rectangle {
                    (int)tilemap.position.x + x * tilemap.tileSize + PLAYER_COLLISION_PADDING, (int)tilemap.position.y + y * tilemap.tileSize + PLAYER_COLLISION_PADDING, tilemap.tileSize, tilemap.tileSize
                }))
                {
...
// NOTE: Multiple versions of DrawTexture() are provided with multiple options
